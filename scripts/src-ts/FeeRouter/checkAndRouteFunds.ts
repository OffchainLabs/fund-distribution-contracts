...
import { assertDefined, DoubleWallet } from "../../template/util";
...
export const checkAndRouteFunds = async (
  ...
  const _submissionFee = await inbox.calculateRetryableSubmissionFee(
    dataLength,
    assertDefined((await parentChainSigner.provider.getFeeData()).gasPrice) // NOTE: I'm not sure why 0 doesn't work here, but it doesn't (on sepolia)
  );
  // add a 20% increase for insurance
  const submissionFee = _submissionFee * (120n) / (100n);

  const currentGasgasPrice = assertDefined((await childChainSigner.provider.getFeeData()).gasPrice);
  ...
  const l1TxRec = new L1TransactionReceipt(await parentChainSigner.v5.provider.getTransactionReceipt(rec!.hash));
